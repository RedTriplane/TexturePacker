...
public class TexturePackerFileProcessor extends FileProcessor {
	...
	public TexturePackerFileProcessor(Settings defaultSettings,
			String packFileName) {
		this.defaultSettings = defaultSettings;

		if (packFileName.toLowerCase().endsWith(
				defaultSettings.atlasExtension.toLowerCase()))
			packFileName = packFileName.substring(0, packFileName.length()
					- defaultSettings.atlasExtension.length());
		this.packFileName = packFileName;

		addInputSuffix(".png", ".jpg");
	}

	public ArrayList<Entry> process(File png_input_dir, File atlas_output_dir)
			throws Exception {
		root = png_input_dir;
		return super.process(png_input_dir, atlas_output_dir);
	}

	private void merge(Settings settings, File settingsFile) {
		try {
			json.readFields(settings,
					new JsonReader().parse(new FileReader(settingsFile)));
		} catch (Exception ex) {
			throw new GdxRuntimeException("Error reading settings file: "
					+ settingsFile, ex);
		}
	}
	...
	public ArrayList<Entry> process(Collection<File> files, File outputRoot)
		...
		if (outputRoot.exists()) {
			...
			File settingsFile = FileWrapper.file(root, "pack.json");
			...
			for (int i = 0, n = rootSettings.scale.length; i < n; i++) {
				...
				String scaledPackFileName = rootSettings.getScaledPackFileName(
						packFileName, i);
				File packFile = FileWrapper.file(root, scaledPackFileName);// !!!! no
																	// root?
				...
				deleteProcessor.addInputRegex("(?i)" + prefix
						+ "\\d*\\.(png|jpg)");
				deleteProcessor.addInputRegex("(?i)" + prefix + "\\.atlas");

				String dir = packFile.parent().getName();
				if (dir == null)
					deleteProcessor.process(outputRoot, null);
				else if (outputRoot.child(dir).exists()) //
					deleteProcessor.process(outputRoot.child(dir), null);
			}
		}
		return super.process(files, outputRoot);
	}

	protected void processDir(Entry inputDir, ArrayList<Entry> files)
			throws Exception {
		...
		// Pack.
		if (!settings.silent)
			L.d(inputDir.inputFile.getName());
