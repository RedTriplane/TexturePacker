...
public class ImageProcessor {
	...
	public ImageProcessor (File rootDir, Settings settings) {
		...
		if (rootDir != null) {
			rootPath = rootDir.toJavaFile().getAbsolutePath().replace('\\', '/');
	...
	public void addImage (File file) {
		BufferedImage image;
		try {
			image = ImageIO.read(file.toJavaFile());
		} catch (IOException ex) {
			throw new RuntimeException("Error reading image: " + file, ex);
		}
		if (image == null) throw new RuntimeException("Unable to read image: " + file);

		String name = file.toJavaFile().getAbsolutePath().replace('\\', '/');
		...
		if (rect != null && settings.limitMemory) rect.unloadImage(file.toJavaFile());
	}
	...
	public Rect addImage (BufferedImage image, String name) {
		...
		if (rect == null) {
			if(!settings.silent) L.d("Ignoring blank input image: " + name);
			return null;
		}
		...
		if (settings.alias) {
			...
			if (existing != null) {
				if (!settings.silent) L.d(rect.name + " (alias of " + existing.name + ")");
