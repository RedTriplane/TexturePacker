...
public class TextureUnpacker {
	...
	public void splitAtlas (TextureAtlasData atlas, String outputDir) {
		...
		if (!outputDirFile.exists()) {
			...
			L.d(String.format("Creating directory: %s", outputDirFile.getPath()));
		}
		...
		for (Page page : atlas.getPages()) {
			...
			for (Region region : atlas.getRegions()) {
				L.d(String.format("Processing image for %s: x[%s] y[%s] w[%s] h[%s], rotate[%s]", region.name,
				...
				if (region.page == page) {
					...
					if (!imgDir.exists()) {
						L.d(String.format("Creating directory: %s", imgDir.getPath()));
						imgDir.mkdirs();
					}
